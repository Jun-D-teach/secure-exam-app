...
import { generateId, readSheet, addRow, deleteRow, SHEETS, findByField } from "../db/sheets";
...
router.post("/", async (req, res) => {
  try {
    ...
    if (!["teacher", "student"].includes(role)) {
      return res.status(400).json({ error: "Role harus teacher atau student" });
    }
...
router.delete("/:id", async (req, res) => {
  try {
    ...
    const user = await findByField<UserRow>(SHEETS.USERS, "id", id);
    if (!user) {
      return res.status(404).json({ error: "Pengguna tidak ditemukan" });
    }
    
    if (user.role === "admin") {
      return res.status(400).json({ error: "Akun admin tidak bisa dihapus" });
    }
...
router.post("/import", async (req, res) => {
  try {
    ...
    if (items.length > 200) {
      return res.status(400).json({ error: "Maksimal 200 akun per import" });
    }
    ...
    for (const item of items) {
      ...
      if (!/^[a-z0-9_.-]{3,32}$/.test(username)) {
        username = username.replace(/[^a-z0-9_.-]/g, ".").slice(0, 32);
      }
